...
public class SequentialServer implements WebServer {
    ...
    private final HashMap<Path, CachedBytes> cache;
    private final int cacheSize;
    private final int timeout;
    private static final Logger logger = Logger.getLogger( SequentialServer.class.getSimpleName());
    ...
    SequentialServer(int port, int cacheSize, Monitor monitor, int timeout, HashMap<String, String> docRoots) {
        this.port = port;
    ...
    private void requestProcess(Socket connection) {
...
//            connection.setSoTimeout(timeout);
            ...
            logger.log(Level.INFO, "Exception thrown parsing request.", ex);
            try {connection.close();} catch (IOException e) {}
            return ;
        }
        ...
        filePath.normalize();
        String ifModifiedSince = parser.getParam("If-Modified-Since");
        ...
        if (cache.containsKey(filePath)) { // cache hit
            ...
            Instant lastModifiedInstant = cb.getLastModified();
            String lastModified = LocalDateTime.from(lastModifiedInstant).toString();
            if (ifModifiedSince.compareTo(lastModified) >= 0) {
                sendHeader(raw, STATUS_NOT_MODIFIED, true);
            } else {
                byte[] body = cb.getBytes();
                sendResponse(raw, STATUS_OKAY, contentType, body, lastModifiedInstant);
            }
            try {connection.close();} catch (IOException e) {}
            return ;
        }
        ...
        try {
            lastModifiedInstant = Files.getLastModifiedTime(filePath).toInstant();
            lastModified = LocalDateTime.from(lastModifiedInstant).toString();
        ...
        if (ifModifiedSince.compareTo(lastModified) >= 0) {
            sendHeader(raw, STATUS_NOT_MODIFIED, true);
            try {connection.close();} catch (IOException e) {}
            logger.info("Cache miss, not modified.");
            return ;
        }
        ...
        try { // read file and send back, maybe put into cache
            ...
            if (cache.size() < cacheSize) {
                cache.put(filePath, new CachedBytes(fileBytes, lastModifiedInstant));
            }
            sendResponse(raw, STATUS_OKAY, contentType, fileBytes, lastModifiedInstant);
            try {connection.close();} catch (IOException e) {}
    ...
    private void sendResponse(OutputStream raw, String responseCode,
                              String contentType, byte[] content, Instant lastModifiedInstant) {
        try {
            sendHeader(raw, responseCode, false);
            Writer w = new OutputStreamWriter(raw);
            StringBuilder response = new StringBuilder();

            response.append("Last-Modified: ").append(RFC_1123_DATE_TIME.format(lastModifiedInstant))
                    .append("\r\n")
            ...
            logger.log(Level.SEVERE, "Error writing to socket.", ex);
        }
    }
